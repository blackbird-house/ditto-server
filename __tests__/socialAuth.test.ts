...
describe('Social Authentication Module', () => {
  ...
  describe('POST /auth/social', () => {
    it('should authenticate with Google successfully', async () => {
      ...
      expect(mockSocialAuthService.verifySocialToken).toHaveBeenCalledWith('google', 'valid-google-token');
    });
    ...
    it('should reject Apple Sign-In requests', async () => {
      ...
      expect(response.body).toHaveProperty('message', 'Only Google Sign-In is currently supported');
    ...
    it('should return 401 for invalid Google token', async () => {
      ...
      mockSocialAuthService.verifySocialToken.mockRejectedValue(new Error('Invalid Google token'));
      ...
      expect(response.body).toHaveProperty('message', 'Invalid Google authentication token');
    });
    ...
    it('should return 400 for unsupported provider error from service', async () => {
      ...
      mockSocialAuthService.verifySocialToken.mockRejectedValue(new Error('Unsupported authentication provider. Only Google Sign-In is supported.'));
      ...
      expect(response.body).toHaveProperty('error', 'Bad request');
      expect(response.body).toHaveProperty('message', 'Unsupported authentication provider. Only Google Sign-In is supported.');
    });

    it('should return 500 for social authentication failure', async () => {
      // Mock general authentication failure
      mockSocialAuthService.verifySocialToken.mockRejectedValue(new Error('Social authentication failed'));
      ...
      expect(response.body).toHaveProperty('message', 'Social authentication failed');
    });

    it('should return 500 for unexpected errors', async () => {
      // Mock unexpected error
      mockSocialAuthService.verifySocialToken.mockRejectedValue(new Error('Unexpected error'));
      ...
      expect(response.body).toHaveProperty('error', 'Internal server error');
      expect(response.body).toHaveProperty('message', 'Failed to authenticate with social provider');
    });
